...
public class StatesAndCapitals
{
    ...
    public void testStatesAndCapitals() throws IOException
    {
        ...
        StateInfo cardinalState = states.stream().filter(stateInfo -> stateInfo.getStateBird().matches("cardinal")).findAny().orElseThrow(()-> new RuntimeException());
        ...
        Integer totalYearlyPrecipitationAcrossStateCapitals = states.stream().map(StateInfo::getCapital).collect(Collectors.summingInt(CapitalInfo::getAverageYearlyPrecipitationInInches));
        ...
        Map<String, Long> numberOfStatesByTimeZone = states.stream().map(StateInfo::getCapital).collect(Collectors.
                groupingBy(CapitalInfo::getTimeZone,Collectors.counting()));
        ...
        Map<String, Long> numberOfStateCapitalsByTimeZone = states.stream().map(StateInfo::getCapital).collect(Collectors.
                groupingBy(CapitalInfo::getTimeZone,Collectors.counting()));
        ...
        List<String> allDistinctStateBirdsMinusMockingbirds = states.stream().map(StateInfo::getStateBird).filter(s -> !s.contains("mockingbird")).distinct().collect(toList());
        ...
        Integer maxStateElevation = states.stream().map(StateInfo::getHighestElevationInFeet).max(Integer::compare).orElseThrow(()-> new RuntimeException());
        ...
        LocalDate earliestDateStateEnteredUnion = states.stream().map(StateInfo::getDateAdmittedToUnion).min(LocalDate::compareTo).orElseThrow(()-> new RuntimeException());
        ...
        StateInfo stateWithLeastDistanceBetweenHighAndLowPoints = states.stream().min(Comparator.comparing(StateInfo::getHighestElevationInFeet)).orElseThrow(()->new RuntimeException());
        ...
        Map<String, String> stateNameToCapitalNamesMap = states.stream().collect(Collectors.toMap(stateInfo -> stateInfo.getStateName(),stateInfo -> stateInfo.getCapital().getCapitalName()));

        testResults.put("A43", StatesAndCapitalsCheck.adv43(stateNameToCapitalNamesMap));

        // ***** Expert (for people who like a challenge) ***** /// ok No problem *-*
        // NOTE: You will * NOT * be graded on these!

        // E1. Submit a list of all the denonyms that do not contain the state's name in them
        // Use flatMap(), filter()

        List<String> allDenonymsThatDoNotContainStateName = states.stream().flatMap(stateInfo -> stateInfo.getDenonyms().stream().filter(s -> !s.contains(stateInfo.getStateName()))).collect(toList());
...
//        states.stream().filter(stateInfo -> stateInfo.getCapital().getCapitalName().matches("Honolulu"))
//                        .flatMap(stateInfo -> stateInfo.getCapital().getSisterCities().stream().flatMap(s -> Arrays.stream(s.split(",")).filter(s1 -> s.charAt(0)))))
